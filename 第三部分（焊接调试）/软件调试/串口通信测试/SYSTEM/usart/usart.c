...
{	
	...
	HAL_UART_Receive_IT(&UART7_Handler, (u8 *)aRxBuffer, RXBUFFERSIZE);//该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==UART7)//如果是串口7，进行串口7 MSP初始化
	{
		...
		GPIO_Initure.Alternate=GPIO_AF8_UART7;	//复用为USART7
		HAL_GPIO_Init(GPIOE,&GPIO_Initure);	   	//初始化PE7

		GPIO_Initure.Pin=GPIO_PIN_8;			//PE8
		HAL_GPIO_Init(GPIOE,&GPIO_Initure);	   	//初始化PE8
		
#if EN_USART7_RX
		HAL_NVIC_EnableIRQ(UART7_IRQn);		//使能USART7中断通道
...
{ 
	...
	timeout=0;
	while(HAL_UART_Receive_IT(&UART7_Handler, (u8 *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)//一次处理完成之后，重新开启中断并设置RxXferCount为1
	{
	 timeout++; //超时处理
	 if(timeout>HAL_MAX_DELAY) break;	
	}
